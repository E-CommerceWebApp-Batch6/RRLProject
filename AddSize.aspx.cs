    ...
    public partial class AddSize : System.Web.UI.Page
    {
        ...
        private void BindrptrSize()
        {
            using (SqlConnection con = new SqlConnection(ConfigurationManager.ConnectionStrings["ECartOnlineShoppingDB"].ConnectionString))
            {
                ...
                using (SqlCommand cmd = new SqlCommand("select A.*,B.*,C.*,D.*,E.* from tblSizes A inner join tblCategory B on B.CatID = CategoryID inner join tblBrands C on C.BrandID = A.BrandID  inner join tblSubCategory \r\nD on D.SubCatID = A.SubCategoryID inner join tblGender E on E.GenderID = A.GenderID;", con))
                {
